...
export default function SyncPanel() {
  ...
  const filtered = useMemo(() => {
    ...
    return (jobs ?? []).filter((job) => {
      if (
        q &&
        !prettyTaskName(job.task_name).toLowerCase().includes(q) &&
        !job.task_name.toLowerCase().includes(q)
      )
        return false
      if (
        tagFilter !== 'all' &&
        !(tagsByTask[job.task_name] ?? []).some((t) => t.name === tagFilter)
      )
        return false
      return true
    })
  ...
  if (isLoading) cards = <p className="text-muted-foreground text-sm">Loading...</p>
    ...
    cards = <p className="text-muted-foreground text-sm">No sync jobs registered.</p>
  else if (!filtered.length)
    cards = (
      <p className="text-muted-foreground text-sm py-2">No tasks match the current filters.</p>
    )
