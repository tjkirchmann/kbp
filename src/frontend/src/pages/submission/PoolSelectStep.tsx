  ...
  if (pools.length === 0) {
    return (
      <div className="glass-panel rounded-2xl p-10 flex flex-col items-center gap-2 text-center">
        <p className="text-sm font-medium text-foreground">No pools are currently open for submissions.</p>
        <p className="text-xs text-muted-foreground">Check back soon or contact your pool organizer.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Select the pool you want to submit an entry for.</p>
      </div>
        ...
        {pools.map(pool => (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors">{pool.name}</p>
              ...
              <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                ...
                {pool.is_featured && (
                  <span className="px-1.5 py-0.5 rounded-full text-xs font-medium bg-primary/15 text-primary">Featured</span>
                )}
              </div>
            </div>
            {pool.submissions_due_at && (
              <p className="text-xs text-muted-foreground mt-2">
                Due {new Date(pool.submissions_due_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
              </p>
            )}
          </button>
        ))}
