...
const ALL_TIME_BEST = [
  ...
  { rank: '2t', name: "Ty K '20", ppg: 4.90 },
  { rank: '2t', name: "Roll Todd '20", ppg: 4.90 },
  { rank: '4', name: "Grapefruit '20", ppg: 4.88 },
  { rank: '5', name: "Caryl '08", ppg: 4.83 },
  { rank: '6', name: "Chris '13", ppg: 4.80 },
  ...
  { rank: '12', name: "Reynolds '16", ppg: 4.60 },
  ...
  { rank: '18t', name: "Fred '08", ppg: 4.50 },
  { rank: '18t', name: "Schmal '03", ppg: 4.50 },
...
const ALL_TIME_WORST = [
  ...
  { rank: '16', name: "Tiny Tim '22", ppg: 2.10 },
...
function PPGRow({ entry, index, variant }: {
  ...
  return (
    <div className={cn(
      ...
      <span className={cn('flex-1 font-medium', isFirst ? 'text-foreground' : 'text-foreground/90')}>
        {entry.name}
      </span>
      <span className={cn(
        'font-mono text-xs font-semibold tabular-nums',
        variant === 'best'
          ? isFirst ? 'text-success' : 'text-primary/80'
          : isFirst ? 'text-destructive' : 'text-muted-foreground'
      )}>
        {entry.ppg.toFixed(2)}
      </span>
    </div>
...
function ChampionRow({ entry, index }: {
  ...
  return (
    <div className={cn(
      ...
      <span className={cn('flex-1 font-medium', entry.vacated ? 'text-muted-foreground line-through' : 'text-foreground/90')}>
        {entry.champion}
      </span>
      {entry.year === 2025 && (
        ...
        <div className="glass-panel rounded-2xl p-8 w-full flex flex-col gap-4">
          <div className="flex items-center gap-6">
            <div className="shrink-0 flex flex-col gap-1">
              <p className="text-xs font-semibold uppercase tracking-widest text-primary">All Time List</p>
              <p className="text-sm text-muted-foreground">Points per game, normalized for the championship game</p>
            </div>
          ...
          {tab === 'best' && (
            <>
              ...
              {ALL_TIME_BEST.map((entry, i) => (
                <PPGRow key={`${entry.name}-${entry.rank}`} entry={entry} index={i} variant="best" />
              ))}
            </>
          )}
          ...
          {tab === 'worst' && (
            <>
              ...
              {ALL_TIME_WORST.map((entry, i) => (
                <PPGRow key={`${entry.name}-${entry.rank}`} entry={entry} index={i} variant="worst" />
              ))}
            </>
          )}
