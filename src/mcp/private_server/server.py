"""
Private MCP server — stdio transport.

Exposes all 7 shared tools plus the execute_sql raw query tool.
Runs locally via stdio (subprocess of an MCP client like Claude Desktop).
...
- cfbd_* tables only
- 60-second timeout
- Optional CSV export to the results/ directory

Usage:
    python -m private_server.server

Environment:
    MCP_DATABASE_URL — Postgres connection string (defaults to localhost)
    MCP_RESULTS_DIR  — CSV export directory (default: ./results/)
"""
...
server = Server("cfbd-mcp-private")

# ═══════════════════════════════════════════════════════════════════════════
# Tool registry — all 7 shared tools + execute_sql
# ═══════════════════════════════════════════════════════════════════════════
...
TOOLS = [
    ...
    Tool(
        name="execute_sql",
        description="Execute a raw SELECT query against CFBD tables. PRIVATE ONLY. Enforces SELECT-only, cfbd_* tables only, and 60-second timeout. Results can be optionally saved to CSV on the server for large result sets.",
        inputSchema={
            "type": "object",
            "properties": {
                "sql": {
                    "type": "string",
                    "description": "Raw SQL SELECT query. Must reference only cfbd_* tables. 60-second timeout enforced.",
                },
                "file_path": {
                    "type": "string",
                    "description": "Optional: simple filename (e.g. 'output.csv') to save results as CSV in the results/ directory. Use for large result sets.",
                },
            },
            "required": ["sql"],
        },
...
async def main_stdio():
    """Run the private MCP server via stdio transport."""
...
def main() -> None:
    """Entry point."""
    print("Starting CFBD MCP private server (stdio transport)", file=sys.stderr)
    print(f"Database: {os.environ.get('MCP_DATABASE_URL', '(default)')}", file=sys.stderr)
