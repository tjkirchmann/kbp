...
from app.services.admin_config import get_espn_alert_channel, get_espn_rate_limit
...
async def _run_poll() -> dict:
    ...
    for game in live_games:
        ...
        if not allowed:
            logger.warning(
                "ESPN rate limit exhausted, skipping remaining games this tick"
            )
            async with SessionLocal() as db:
                await log_event(
                    db,
...
async def poll_live_espn_games(timestamp: int | None = None) -> dict:
    ...
    try:
        ...
        if result.get("polled", 0) > 0:
            logger.info(
                "ESPN poll: %d/%d games updated",
                result["polled"],
                result.get("live_games", 0),
            )
        return result
    except Exception as exc:
