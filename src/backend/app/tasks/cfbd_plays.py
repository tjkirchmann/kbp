...
async def _sync_plays(db, plays: list[dict], year: int) -> tuple[int, int]:
    ...
    for p in plays:
        ...
        if pid is None:
            continue
        rows[str(pid)] = {
            "id": str(pid), "game_id": p.get("gameId"),
            "drive_id": str(p["driveId"]) if p.get("driveId") is not None else None,
            "season": p.get("season") or year, "week": p.get("week"),
            "season_type": p.get("seasonType"), "offense": p.get("offense"),
            "offense_conference": p.get("offenseConference"), "defense": p.get("defense"),
            "defense_conference": p.get("defenseConference"), "home": p.get("home"),
            "away": p.get("away"), "offense_score": p.get("offenseScore"),
            "defense_score": p.get("defenseScore"), "period": p.get("period"),
            "yard_line": p.get("yardLine"), "yards_to_goal": p.get("yardsToGoal"),
            "down": p.get("down"), "distance": p.get("distance"),
            "scoring": p.get("scoring"), "yards_gained": p.get("yardsGained"),
            "play_type": p.get("playType"), "play_text": p.get("playText"),
            "ppa": p.get("ppa"), "last_synced_at": now,
        }
    values = list(rows.values())
    if values:
        await batch_upsert(db, CfbdPlay, values, _batch(len(values[0])), index_elements=("id",))
    return len(values), 0
...
async def _sync_play_stats(db, stats: list[dict], year: int) -> tuple[int, int]:
    ...
    for s in stats:
        ...
        rows[key] = {
            "play_id": str(play_id), "athlete_id": str(athlete_id),
            "stat_type": stat_type, "game_id": s.get("gameId"),
            "season": s.get("season") or year, "week": s.get("week"),
            "team": s.get("team"), "conference": s.get("conference"),
            "opponent": s.get("opponent"), "athlete_name": s.get("athleteName"),
            "stat": s.get("stat"), "last_synced_at": now,
        }
    values = list(rows.values())
    if values:
        await batch_upsert(
            db, CfbdPlayStat, values, _batch(len(values[0])),
...
async def sync_cfbd_plays(timestamp: int | None = None) -> dict[str, Any]:
    ...
    async with SessionLocal() as db:
        ...
        for endpoint, syncer in _SYNCERS.items():
            ep: dict[str, Any] = {"synced": [], "skipped": [], "errors": [], "processed": 0}
