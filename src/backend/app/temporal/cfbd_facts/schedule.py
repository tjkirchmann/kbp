"""Temporal Schedule for the daily CFBD facts run.

A Schedule is the modern replacement for both the deprecated workflow cron field
and Procrastinate's periodic/catch-up machinery. ``ensure_cfbd_facts_schedule``
...
WORKFLOW_ID = "cfbd-facts"  # fixed → SKIP overlap dedups concurrent runs
...
def _build_schedule() -> Schedule:
    return Schedule(
        ...
        spec=ScheduleSpec(cron_expressions=[settings.temporal_cfbd_facts_cron]),
        policy=SchedulePolicy(overlap=ScheduleOverlapPolicy.SKIP),
    )


async def ensure_cfbd_facts_schedule(client: Client) -> None:
