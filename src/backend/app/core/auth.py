...
async def get_current_user(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Could not fetch user from Clerk",
        )
    ...
    if not primary_email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No email on Clerk user"
        )
    ...
    if user.deleted_at is not None:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Account removed"
        )
    return user
