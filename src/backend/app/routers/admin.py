...
class SyncNotify(BaseModel):
    ...
    startup_stale_seconds: Optional[int] = None  # skip startup defer if succeeded within window
...
class SyncJobStatus(BaseModel):
    ...
    cron: Optional[str] = None          # None for run-only or paused tasks
...
async def _hidden_task_names(db: AsyncSession) -> list[str]:
    ...
    rows = await db.execute(
        select(AdminNotifyConfig.task_name).where(AdminNotifyConfig.hide_in_history.is_(True))
    )
...
async def get_sync_status(db: AsyncSession = Depends(get_db)):
    ...
    for task_name, task in tasks.items():
        ...
        out.append(SyncJobStatus(
            ...
            last_run_at=last_run_at, next_run_at=next_run_at, runs=runs, notify=notify,
        ))
...
async def get_recent_runs(
    ...
    rows = (await db.execute(
        _GLOBAL_RUNS_SQL, {"before_id": before_id, "limit": limit, "hidden": hidden}
    )).mappings().all()
    out: list[GlobalRun] = []
    for r in rows:
        started, ended = r["started_at"], r["ended_at"]
        duration = (ended - started).total_seconds() if started and ended else None
        out.append(GlobalRun(
...
async def get_run_detail(job_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return RunDetail(
        id=job["id"], task_name=job["task_name"], status=job["status"],
        queue_name=job["queue_name"], priority=job["priority"], attempts=job["attempts"],
        lock=job["lock"], queueing_lock=job["queueing_lock"],
        scheduled_at=job["scheduled_at"], abort_requested=job["abort_requested"],
        worker_id=job["worker_id"], args=job["args"] or {},
        started_at=started, ended_at=ended, duration_seconds=duration, events=events,
    )
...
async def retry_run(job_id: int):
    ...
    await procrastinate_app.job_manager.retry_job_by_id_async(
        job_id, retry_at=datetime.now(timezone.utc)
    )
    return {"ok": True}
...
async def get_task_detail(task_name: str, db: AsyncSession = Depends(get_db)):
    ...
    stats = TaskStats(
        ...
        avg_duration_seconds=avg_d, p95_duration_seconds=p95_d,
    )
...
class NotifyConfigUpdate(BaseModel):
    ...
    startup_stale_seconds: Optional[int] = None  # -1 clears to null ("Always")
    hide_in_history: Optional[bool] = None       # exclude runs from the History rail
...
async def update_notify_config(
    ...
    if body.channel_name == "":
        ...
        exists = (await db.execute(
            select(NotificationChannel.name).where(NotificationChannel.name == body.channel_name)
        )).scalar_one_or_none()
        if exists is None:
            raise HTTPException(status_code=400, detail=f"Unknown channel {body.channel_name!r}")
...
class CronUpdate(BaseModel):
    cron: Optional[str] = None   # null or blank => pause (task won't fire)
...
async def update_cron(task_name: str, body: CronUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if cron is not None:
        try:
            next_run_at = croniter(cron, datetime.now(timezone.utc)).get_next(datetime)
        except Exception:
            next_run_at = None
...
async def list_channels(db: AsyncSession = Depends(get_db)):
    ...
    return [ChannelSchema(name=c.name, strategy=c.strategy, config=c.config or {}) for c in rows]


@router.put("/notify/channels/{name}", response_model=ChannelSchema)
async def upsert_channel(name: str, body: ChannelUpsert, db: AsyncSession = Depends(get_db)):
    from sqlalchemy.dialects.postgresql import insert as pg_insert
    stmt = pg_insert(NotificationChannel).values(
        name=name, strategy=body.strategy, config=body.config
...
async def test_channel(name: str, db: AsyncSession = Depends(get_db)):
    ...
    await notification_service.notify(
        strategy=chan.strategy, config=chan.config or {}, event="success",
...
async def add_entity_tag(
    entity_type: str, entity_id: str, body: TagCreate,
    db: AsyncSession = Depends(get_db),
):
    from fastapi import HTTPException
    try:
