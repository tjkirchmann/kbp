...
down_revision: str | None = "s8b9c0d1e2f4"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "coach_id",
            sa.String(),
            sa.ForeignKey("cfbd_coaches.coach_id", ondelete="CASCADE"),
            primary_key=True,
        ),
    ...
    op.execute(
        "UPDATE admin_notify_config SET cron = NULL WHERE task_name = 'cfbd_teams'"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        sa.text(
            "UPDATE admin_notify_config SET cron = :cron WHERE task_name = 'cfbd_teams'"
        ).bindparams(cron=_DIMS_CRON)
    )
